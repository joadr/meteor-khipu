...
KhipuAtmosphere.statusServer = {
    ...
    subscribe: function (options) {
        ...
        var statusRequest = $.extend({}, defaults, options);
...
KhipuLib = {
    ...
    onIsMobile: function () {
        if (KhipuLib.options.elementId != null) {
            var element = $('#' + KhipuLib.options.elementId);
    ...
    hideWaitAppModal: function () {
        $('.khipu-wait-app-modal').remove();
        $('.khipu-wait-app-modal-overlay').remove();
    },
    showModal: function (options) {
        var modalName = 'khipu-modal-window-' + KhipuLib.makeId();
        var overlay = document.createElement('div');
        overlay.id = modalName + 'overlay';
        $(overlay).css({width: '100%', height: '100%', top: 0, left: 0, position: 'fixed', 'opacity': '0.8', 'background-color': '#000000', 'z-index': 500});
        document.body.appendChild(overlay);
        var modal = document.createElement('div');
        modal.id = modalName;
        if (options.classes) {
            $(modal).addClass(options.classes);
            $(overlay).addClass(options.classes + '-overlay');
        }
        $(modal).css({'display': 'none',
            ...
            'position': 'fixed',
            'z-index': 10000,
            'left': Math.max(0, (($(window).width() - options.width) / 2) + $(window).scrollLeft()) + "px",
            'top': Math.max(0, (($(window).height() - options.height) / 2) + $(window).scrollTop()) + "px",
            'border-radius': '5px',
        ...
        $(modalWrapper).css({'padding': '30px'});
        modal.appendChild(modalWrapper);

        document.body.appendChild(modal);

        var contentWrapper = document.createElement('div');
        $(contentWrapper).html(options.html);
